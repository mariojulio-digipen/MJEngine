...
vec4 computeLighting(int lightIndex)
{
	...
	{
		Kd = vec3(0.3f, 0.8f, 0.1f)  + Fraglights[lightIndex].LightAmbient;
	}
	...
	if(Lights == 1)
	{
		...
		else
		{
			...
			myFragColor = vec4(da_color, textureAlpha) * 0.5;
			//if(IsCloth == 1)
			//	myFragColor = vec4(N, 1.0f);
		}
		...
		//if(dot(V,N) == 0)
		//{
		//	// polygon is perpendicular, therefore, siloutte.
			
		//	myFragColor = vec4(1.0f, 0.0f, 0.0f, 1.0f);
		//}
		
			//myFragColor = vec4(vec3(dot(V,N)), 1.0f);
	}
